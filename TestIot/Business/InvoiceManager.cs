...
namespace TestIot.Business
{
    class InvoiceManager
    {
        ...
        public void InsertMessages(List<Payload> messages)
        {
            using (var db = new ESuiteEntities())
            {
                foreach (var message in messages)
                {
                    var existingMessage = db.Messages.Where(m => m.ID == message.Id).FirstOrDefault();
                    if (existingMessage != null) continue;
                    var newMessage = new Message
                    {
                        ID = message.Id,
                        SENTTIME = (decimal)message.SentTime
                    };
                    db.Messages.Add(newMessage);
                    CreateInvoice(message.MeterId, message.BillingFromDate, message.BillingToDate,
                                    message.LastMeterReading, message.CurrentMeterReading);
                    db.SaveChanges();
                }
            }
        }
        ...
        public void CreateInvoice(ESuiteEntities db, string meterId, DateTime billingFromDate, DateTime billingToDate,
